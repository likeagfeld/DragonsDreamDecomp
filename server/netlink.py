...
class Netlink:
    ...
    def digit_parser(self):
        ...
        if raw_string == "0":
            ...
            # Config-driven server dial code (e.g. 698 for Dragon's Dream)
            self.ms = "calling"
            self.mode = "netlink"
            self.dial_string = raw_string
            return {'client':self.mode, 'dial_string':raw_string}
    ...
    def do_dragons_dream(self, server_cfg):
        """
        Specialized handler for Dragon's Dream (and similar BBS-protocol games).
        Answers modem, connects to TCP server, transparently proxies ALL data.
        Does NOT flush serial buffer after CONNECT - the Saturn sends BBS
        commands (" P\r", "SET\r", "C HRPG\r") immediately and the server
        must see them.
        """
        host = server_cfg.get('host', '192.168.50.180')
        port = int(server_cfg.get('port', '8020'))
        label = server_cfg.get('name', "Dragon's Dream")
        ...
        # 2. Connect to the TCP Server IMMEDIATELY
        # This is key: the Saturn will immediately send BBS commands.
        # We need the Server to see those, not the Pi script.
        try:
            s = socket.socket()
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            s.settimeout(5)
            s.connect((host, port))
            self.logger.info("%s: Tunnel Established", label)
            s.setblocking(False)
        except Exception as e:
            self.logger.warn("%s: Connection failed: %s" % (label, e))
            return
        ...
        while True:
            ...
            # Check for hangup (Carrier Detect)
    ...
    def mode_handler(self):
        if self.mode == "netlink":
            # Check config-driven server routing
            ...
            if server:
                ...
                if handler_type == 'transparent':
                    ...
                    self.do_dragons_dream(server)
                else:
                    # Standard server handler (flushes serial, optional auth)
